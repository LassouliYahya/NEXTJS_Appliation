...
exports.deletePost = async (req, res) => {
    const {_id}=req.post;
    if (!req.isPosted) {
        return res.status(400).json({message:"Your Not Authorized to action !!"});
                } 
...
exports.getPostById = async(req,res,next,id) => {
    ...
    const postId=mongoose.Types.ObjectId(req.post.postedBy._id)

    if ( req.user && postId.equals(req.user._id)) {
        req.isPosted=true; 
        return next();
    }
   next();
